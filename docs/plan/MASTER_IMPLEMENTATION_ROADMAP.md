...
| Phase | Name | Status (refreshed 2026-09-21 for Phase 2; other rows keep their 2026-09-20 verification) | Key tasks (IDs) | Dependencies | Blocking items | Evidence of state | Exit criteria (short) |
|---|---|---|---|---|---|---|---|
...
| 2 | Shared infrastructure (P1) | 🟨 in progress — **NWB-P1-001 (queue + scheduler + worker base) DONE 2026-09-21**, **NWB-P1-002 (audit formalization + query API) DONE 2026-09-21**; `bun test` 531/0 (287 pass / 258 skip with no `DATABASE_URL`), backlog ticket book at `.scratch/p1-shared-infra/` | NWB-P1-001 ✅, NWB-P1-002 ✅, NWB-P1-003…012; new: **NWB-P1-013** (purge batching, found by P1-001), **NWB-P1-014** (audit hash chain) + **NWB-P1-015** (audit anonymization, F-29), both split out of P1-002 | Phase 1; **D6** | Resend/R2 credentials | plan §5 P1; §12, §12.1 | §12 exit gate: worker loop ✅, purge/reclamation scheduled ✅, real email, approval queue, media, flags, observability |
| 3 | Social accounts (P2, Mod 3) | ✅ **DONE 2026-09-25** (row refreshed 2026-09-26; PR #22 + #23) — exit gate met on the engineering side. Follow-up **NWB-P2-007** 🟨 2026-09-26 for the lifecycle gaps P2-006 scoped out (pause/resume, on-demand probe, attention filter, impact preview, FR-SOC-008/022 notifications) and two bugs found in it | NWB-P2-001…006 ✅ (`.scratch/p2-social-accounts/issues/01…06`), NWB-P2-007 🟨 (`issues/07`) | Phase 2 ✅ | platform API credentials — **operator residual**: an unconfigured platform refuses `initiate` with a validation error, it does not half-start | `.scratch/p2-social-accounts/spec.md` (exit-gate evidence) + `issues/01…07`; roadmap §13 rows | 5 platforms connect/refresh/healthy; breaker trips+recovers — **966/966 tests**, coverage 93.8% services / 96.8% lib |
