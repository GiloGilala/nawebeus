...
- **Current state:** `inviteMember`/`bulkInviteMembers` create invited rows + email link `${CORS_ORIGIN}/invite?token=…`; no accept endpoint, no token verification path for the invitee.
