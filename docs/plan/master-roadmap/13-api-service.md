...
**Cross-cutting gaps:** pagination on all list endpoints (**F-14 — done 2026-09-21, NWB-P0-030**; **cursor-based**, not `?page&limit` as this line originally said — see **D-17**: `docs/technical/API Reference.md` §2.6 mandates cursors and wins. `?limit` default 20 / max 100 plus an opaque base64url `cursor`, surfaced as `meta.pagination = {cursor, hasMore}` in the envelope's existing `meta` slot. Live on members, admin users, sessions and API keys; Phase 7's list screens consume the cursor when they land); rate limiting on public endpoints (Phase 1 limiter is the only implementation — P11 entry, P13 webhooks, P2 callbacks all reuse it per plan §7 "idempotency/public writes"); error contract for 429 (existing `RateLimitError` + `retryAfter` — keep).
