...
- **Without `DATABASE_URL`** `bun test` runs the pure suites (232 pass, ~0.2 s) and cleanly **skips the 211 database-gated tests** (`describe.skipIf(!hasDb())`). It never goes red for lack of a database.
- **With `DATABASE_URL`** pointing at a pushed and seeded database, the full suite runs (432 tests across 44 files, ~35 s).
