...
- **Without `DATABASE_URL`** `bun test` runs the pure suites (227 pass, ~0.2 s) and cleanly **skips the 196 database-gated tests** (`describe.skipIf(!hasDb())`). It never goes red for lack of a database.
- **With `DATABASE_URL`** pointing at a pushed and seeded database, the full suite runs (412 tests across 43 files, ~30 s).
