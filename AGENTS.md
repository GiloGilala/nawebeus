...
- Local reproduction of the `test` job: create a fresh database, then run `db:push` (with `DB_*`), `seed`, and `bun test` with `DATABASE_URL` set. A fresh database is required, because push is not idempotent. Step-by-step, including a no-install PostgreSQL 14: `docs/agents/local-database.md`.
