...
src/server/index.ts       ← Hono app factory (CORS, request context + access log, error handler,
                            route mounting, `/api/health` readiness probe)
  api/                    ← Hono route handlers, mounted at /api (thin: validate + delegate)
    ...
    media/   /media (multipart upload, library list), /media/:assetId (metadata,
    ...
    social/  /social/oauth/:platform/initiate (authed, socialaccounts.connect) and
            ...
            management surface (NWB-P2-006): GET /social/accounts (list — keyset
            (connected_at, id), token-free, platform/status filters, disconnected hidden
            unless named), GET /social/accounts/:accountId (detail: breaker + last error +
            quota snapshot + latest health), GET .../health (newest-first timeline),
            GET /social/usage (org roll-up, manager+), DELETE /social/accounts/:accountId
            (admin; typed-username confirm in the BODY; best-effort adapter revoke recorded,
            never thrown; tokens NULLed; retention = now()+90d; repeat → 404)
            lifecycle surface (NWB-P2-007) — no new CASL verbs: POST .../pause and
            POST .../resume (FR-SOC-016, socialaccounts.connect = manager+), POST
            .../health-check (on-demand probe; a success closes an open breaker — P2-003's
            recovery path made operator-reachable), GET .../usage (the per-account
            getQuotaSnapshot, socialaccounts.usage), GET /social/accounts?attention=true
            (+ meta.attentionCount, FR-SOC-044's data half), DELETE ...?dryRun=true
            (FR-SOC-011's impact preview: writes nothing, so it asks for no confirmation)
...
src/server/middleware/    ← Hono middleware
  ...
  request-context.ts ← one correlation id per request (honored/echoed x-request-id)
                            + the access log; registered FIRST in both app factories
  error-handler.ts
src/lib/request-context.ts ← the request-scope ALS behind that id (writeAuditLog defaults
                            its request_id from it; never throws outside a request)
src/server/health.ts     ← collectHealth: DB ping + queue depth → { ok | degraded | error }
src/services/             ← Business logic (single source of truth)
  ...
  storage/ types (`StorageTransport`, structural `S3ClientLike`, `storage_url = <driver>://<key>`
  ...
  social/  platforms (DEC-009 registry: OAuth endpoints/scopes/PKCE per platform),
           oauth-client (generic code-for-token exchange, injectable fetch, zero
           per-platform branches), adapters/ (NWB-P2-005 — one PlatformAdapter per
           platform: profile dialect, probe shaping incl. Reddit's User-Agent, Meta
           pair's native refresh grants, optional revokeRequest hook for the
           disconnect's best-effort revocation — YouTube + Reddit + X, which is
           RFC 7009 and needs the token_type_hint), service (single-use state machine,
           sealed tokens, connect/reconnect, state purge, quota ledger, management
           reads + disconnect with the 90-day retention stamp, and NWB-P2-007's
           lifecycle half: pause/resume, on-demand probe, impact preview, attention
...
- **A social account's lifecycle is guarded in the service, and a paused account is not a dispatchable one** (NWB-P2-007) — pause/resume (`setAccountCollection`) is a narrow state machine: pause only from `active`/`error`, resume only from `paused`, everything else a 409 `SOCIAL_ACCOUNT_STATE_CONFLICT`. A `needs_reauth` account can be neither paused nor resumed, because FR-SOC-016's "resume without re-authentication" is about pausing — resuming a dead token would dress it up as healthy. Pausing keeps the sealed tokens (that is the difference from disconnecting) and flips `is_active`, which is what actually stops the machinery: the `*/5` refresh sweep selects `status = 'active' AND is_active = true`, and **`assertDispatchAllowed` refuses a paused row with a 409** (not the breaker's 503 — a retry will not fix it, a human has to resume it). Both transitions write a `social_account_health_log` row (the timeline FR-SOC-042 reads would otherwise lie) and one audit row naming the actor. Two invariants to keep: `markNeedsReauth` writes its health-log row, its audit event and its notification **only on a real transition** (`chk_sahl_transition_differs` rejects `previous_status = status`, and a 23514 there aborts the caller's whole transaction), and the reconnect revive clears *every* remnant of the outage — breaker, `is_active`, retention stamp — or a freshly re-authenticated account is born undispatchable.
- **Social notifications ride the email channel through a port, and can never fail their trigger** (NWB-P2-007) — `SocialNotifier` is one function type over three events (`connected` / `reconnected` / `needs_reauth`); `defaultSocialNotifier` (`src/services/social/notifier.ts`) resolves `owner` + `admin` seats (DEC-039's administrative tiers — `manager` is operational and only receives `needs_reauth` as the account's Primary Manager, `social_accounts.primary_manager_id`) plus that Primary Manager, requires a verified address, and sends via P1-004's `emailService`. `alert_rule_source` has no `social` value, so `fireAlert` would cost a migration for a notification; P6 swaps the implementation, not the call sites. The service awaits it through a `notify()` helper that try/catches and logs — a dead SMTP provider must not make it impossible to connect an account. `setSocialNotifierForTest` is the seam.
- **Every 500 is opaque by default** — `errorHandler` maps any non-`AppError` to a generic `INTERNAL_ERROR`, so the real cause never reaches the client. The *server* always logs a structured line with the real cause since NWB-P1-012 (see the request-context fact below); `NWB_DEBUG_ERRORS=1` additionally logs stacks for any error and dumps 4xx errors at `warn` — it never changes what the client sees.
- **One correlation id per request, from log to audit row** (NWB-P1-012) — `src/server/middleware/request-context.ts` (registered **first** in both app factories) honors an inbound `x-request-id` when it is 1..100 chars (the `unified_audit_log.request_id` width — anything longer is replaced, never forwarded into the column), else mints `req_<uuid>`; it is echoed on every response, carried in an AsyncLocalStorage (`src/lib/request-context.ts`), and **`writeAuditLog` defaults `request_id` from it** — explicit params win (the DSAR self-cite keeps its domain id), and workers/CLI (no request scope) keep writing NULL. One `logger.info("request", …)` line per request carries `requestId, method, route, status, durationMs` (+ `orgId/userId/authMethod` when authenticated); `/api/health` probes and `OPTIONS` preflights are excluded as infrastructure traffic. **`route` is the endpoint template from the last non-catch-all entry of `c.req.matchedRoutes` — never `c.req.routePath`** (frame-local: `/api/audit/*` for a request aborted in route middleware, `/*` on a 404) and never the raw path, because path params can be live single-use tokens (the invitation token). `GET /api/health` is a real readiness probe: DB `SELECT 1` failure → **503** `error`; queue `disabled`/`ok`(+depth)/`error`, where a started-but-failing queue is `degraded` but **still 200** (ADR-007: the API serves without its queue). Tests: `src/tests/observability.test.ts`; the shared logger is silenced in `src/tests/preload.ts` (assert via `spyOn(logger, …)`; `realLogWriters` are the un-silenced originals).
