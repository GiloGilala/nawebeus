...
export const rolePermissions = pgTable(
  ...
  (table) => [
    ...
    index(`${tablePrefix}role_permissions_permission_id_idx`).on(
      table.permissionId,
    ),
    index(`${tablePrefix}role_permissions_assigned_by_idx`).on(
      table.assignedBy,
    ),
    ...
    index(`${tablePrefix}role_permissions_template_id_idx`).on(
      table.templateId,
    ),

    index(`${tablePrefix}role_permissions_valid_from_idx`).on(table.validFrom),
    index(`${tablePrefix}role_permissions_valid_until_idx`).on(
      table.validUntil,
    ),
    ...
    // COMPOSITE INDEXES
    // ============================================
    index(`${tablePrefix}role_permissions_active_idx`)
      .on(table.roleId, table.status, table.allowed)
      ,

    index(`${tablePrefix}role_permissions_priority_active_idx`)
      .on(table.roleId, table.priority, table.allowed)
      ,
