...
export const sessions = pgTable(
  `${tablePrefix}sessions`,
  {
    ...
    impersonationApprovedBy: uuid('impersonation_approved_by').references(
      () => users.id,
      { onDelete: 'set null' },
    ),
    ...
    // ============================================
    sessionTokenHash: varchar('session_token_hash', { length: 512 })
      .notNull()
      .unique(),
    ...
    loginMethod: loginMethodPgEnum('login_method')
      .notNull()
      .default('password'),
    ...
    suspiciousActivityDetected: boolean('suspicious_activity_detected').default(
      false,
    ),
    ...
    previousRefreshJtiGraceUntil: timestamp(
      'previous_refresh_jti_grace_until',
      {
        withTimezone: true,
        mode: 'date',
      },
  ...
  (table) => [
    ...
    // UNIQUE INDEXES
    // ============================================
    uniqueIndex(`${tablePrefix}sessions_token_hash_unique_idx`).on(
    ...
    index(`${tablePrefix}sessions_external_session_id_idx`).on(
      table.externalSessionId,
    ),
    index(`${tablePrefix}sessions_revoked_by_idx`).on(table.revokedBy),

    // Impersonation indexes
    index(`${tablePrefix}sessions_impersonated_user_idx`).on(
      table.impersonatedUserId,
    ),
    index(`${tablePrefix}sessions_impersonated_by_user_idx`).on(
      table.impersonatedByUserId,
    ),
    index(`${tablePrefix}sessions_impersonation_approved_by_idx`).on(
      table.impersonationApprovedBy,
    ),

    index(`${tablePrefix}sessions_parent_session_idx`).on(
      table.parentSessionId,
    ),
    index(`${tablePrefix}sessions_replaced_by_session_idx`).on(
      table.replacedBySessionId,
    ),
    ...
    index(`${tablePrefix}sessions_absolute_timeout_idx`).on(
      table.absoluteTimeoutAt,
    ),
    index(`${tablePrefix}sessions_risk_calculated_idx`).on(
      table.riskCalculatedAt,
    ),
    index(`${tablePrefix}sessions_authentication_level_idx`).on(
      table.authenticationLevel,
    ),
    ...
    index(`${tablePrefix}sessions_cleanup_idx`).on(
      table.expiresAt,
      table.status,
    ),

    index(`${tablePrefix}sessions_user_risk_idx`).on(
      table.userId,
      table.riskLevel,
      table.status,
    ),

    index(`${tablePrefix}sessions_user_remember_idx`).on(
      table.userId,
      table.rememberMe,
      table.status,
    ).where(sql`
    ...
    // JSONB GIN INDEXES
    // ============================================
    index(`${tablePrefix}sessions_metadata_gin_idx`).using(
      'gin',
      table.metadata,
    ),
  ],
