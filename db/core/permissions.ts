...
export const permissions = pgTable(
  `${tablePrefix}permissions`,
  {
    ...
    permissionString: varchar('permission_string', { length: 255 })
      .notNull()
      .unique(),
    ...
    // ============================================
    parentPermissionId: uuid('parent_permission_id').references(
      (): any => permissions.id,
      { onDelete: 'set null' },
    ),
  ...
  (table) => [
    ...
    index(`${tablePrefix}permissions_organization_idx`).on(
    ...
    index(`${tablePrefix}permissions_parent_permission_idx`).on(
      table.parentPermissionId,
    ), // NEW
...
export const permissionGroups = pgTable(
  ...
  (table) => [
    ...
    index(`${tablePrefix}permission_groups_organization_idx`).on(
      table.organizationId,
    ),
