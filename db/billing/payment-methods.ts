...
export const cardFundingEnum = pgEnum('card_funding', [
  'credit',
  'debit',
  'prepaid',
  'unknown',
...
export const cvcCheckEnum = pgEnum('cvc_check', [
  'pass',
  'fail',
  'unchecked',
  'unavailable',
...
export const paymentMethods = pgTable(
  `${tablePrefix}payment_methods`,
  {
    ...
    networkTokenStatus: networkTokenStatusEnum('network_token_status').default(
      'disabled',
    ),
    ...
    addressLine1Check: addressCheckEnum('address_line1_check').default(
      'unchecked',
    ),
    addressPostalCodeCheck: addressCheckEnum(
    ...
    strongCustomerAuthRequired: boolean(
      'strong_customer_auth_required',
    ).default(false),
  ...
  (table) => [
    ...
    index('payment_methods_processor_payment_method_id_idx').on(
      table.processorPaymentMethodId,
    ),
    index('payment_methods_processor_customer_id_idx').on(
    ...
    index('payment_methods_verification_status_idx').on(
    ...
    index('payment_methods_org_active_idx').on(
    ...
    index('payment_methods_org_default_lookup_idx').on(
      table.organizationId,
      table.isDefault,
      table.status,
    ).where(sql`
    ...
    index('payment_methods_expiring_soon_idx').on(table.expiresAt, table.status)
      .where(sql`
        status = 'active' 
        AND deleted_at IS NULL
    ...
    // Expired (for cleanup)
    index('payment_methods_expired_cleanup_idx').on(table.expiresAt).where(sql`
        status = 'active' 
        AND deleted_at IS NULL
        AND expires_at IS NOT NULL
        AND expires_at < now()
    ...
    index('payment_methods_verification_pending_idx').on(
      table.status,
      table.isVerified,
      table.createdAt,
    ).where(sql`
    ...
    index('payment_methods_blocked_active_idx').on(
      table.blockStatus,
      table.organizationId,
    ).where(sql`
    ...
    index('payment_methods_high_risk_idx').on(table.riskLevel, table.status)
      .where(sql`
    ...
    index('payment_methods_unused_idx').on(table.lastUsedAt, table.status)
      .where(sql`
        status = 'active'
        AND deleted_at IS NULL
    ...
    // Update reminders
    index('payment_methods_update_reminder_idx').on(
      table.updateRequestedAt,
      table.updateRemindersSent,
    ).where(sql`
    ...
    index('payment_methods_tags_gin_idx')
