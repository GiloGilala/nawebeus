...
export const payments = pgTable(
  `${tablePrefix}payments`,
  {
    ...
    creditAmount: decimal("credit_amount", { precision: 20, scale: 2 }).default("0.00"),
    ...
    platformFee: decimal("platform_fee", { precision: 20, scale: 2 }).default("0.00"),
    ...
    feeBreakdown: jsonb("fee_breakdown")
      .$type<
        Array<{
          type: "processor" | "network" | "tax" | "currency_conversion" | "application" | "other";
          name: string;
          amount: number;
          currency: string;
          rate?: number;
    ...
    paymentMethodSnapshotVersion: integer("payment_method_snapshot_version").default(1),
  ...
  (table) => [
    ...
    // Billing period
    index("payments_billing_period_idx").on(table.billingPeriodStart, table.billingPeriodEnd),
    ...
    index("payments_org_method_idx").on(table.organizationId, table.paymentMethod),
    ...
    index("payments_retry_pending_idx").on(table.status, table.nextRetryAt, table.attemptCount),
    ...
    index("payments_org_recent_idx").on(table.organizationId, table.createdAt, table.status),
    ...
    // Settlement tracking
    index("payments_settlement_pending_idx")
      .on(table.settlementStatus, table.succeededAt)
      .where(sql`
    ...
    // Disputes
    index("payments_dispute_status_idx").on(table.disputeStatus, table.disputeOpenedAt),

    // Chargebacks
    index("payments_chargeback_status_idx").on(table.chargebackStatus, table.chargebackAt),
    ...
    index("payments_tags_gin_idx").using("gin", table.tags).where(sql`tags IS NOT NULL`),
