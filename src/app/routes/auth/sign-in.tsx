...
function SignInPage() {
  ...
  return (
    <main style={{ maxWidth: 420, margin: "2rem auto", fontFamily: "sans-serif" }}>
      <h1>Sign in</h1>
      <p style={{ color: "#666" }}>
        Server Function: `signinServerFn` → `services/auth/auth.service`.
      </p>
      <form
        onSubmit={onSubmit}
      ...
      <p style={{ fontSize: "0.8rem", color: "#888", marginTop: "1rem" }}>
        Mobile calls <code>POST /api/auth/signin</code> (Hono, <code>src/server/api/auth</code>)
        with the same service. Web goes direct.
      </p>
    </main>
  );
}
